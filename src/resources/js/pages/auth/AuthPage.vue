...
async function handleGoogleLogin() {
  const dummyEmail = prompt("Simulasi Google Sign-In\n\nMasukkan Email Google untuk simulasi:", "budi.google@gmail.com")
  if (!dummyEmail) return

  const dummyName = prompt("Masukkan Nama Google untuk simulasi:", "Budi Google")
  if (!dummyName) return

  const dummyId = prompt("Masukkan Google ID (sub) untuk simulasi:", "google_123456789")
  if (!dummyId) return

  isLoading.value = true
  showToast('Memproses Login Google…')

  try {
    const payload = {
      id_token: `mock_google_token_${dummyId}`,
      email: dummyEmail,
      name: dummyName
    }
