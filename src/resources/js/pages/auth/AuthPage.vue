<template>
  <div style="display:flex;min-height:100vh;align-items:stretch;">
    ...
    <div v-if="showMockGoogleModal" class="google-modal-overlay" @click.self="showMockGoogleModal = false">
      <div class="google-modal">
        <div class="google-modal-logo">
        ...
        <div v-if="!showGoogleCustomForm">
          <h3 class="google-modal-title">Pilih akun</h3>
          <p class="google-modal-subtitle">untuk melanjutkan ke Kulaan.id</p>
          
          <div class="google-accounts-list">
            <div v-for="acc in mockGoogleAccounts" :key="acc.email" class="google-account-item" @click="selectMockGoogleAccount(acc)">
              <div class="google-account-avatar">{{ acc.name[0] }}</div>
            ...
            <div class="google-account-item google-use-other" @click="showGoogleCustomForm = true">
              <div class="google-account-avatar other-avatar">👤</div>
              <div class="google-account-info">
                <div class="google-account-name">Gunakan akun lain</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-else>
          <h3 class="google-modal-title">Gunakan akun lain</h3>
          <p class="google-modal-subtitle">Masukkan info profil Google</p>
          
          <form @submit.prevent="submitGoogleCustomAccount" class="google-custom-form">
            <div class="google-input-group">
              <input type="text" v-model="googleCustomForm.name" placeholder="Nama Lengkap" required class="google-input" />
            </div>
            <div class="google-input-group">
              <input type="email" v-model="googleCustomForm.email" placeholder="Email Google" required class="google-input" />
            </div>
            <div class="google-custom-actions">
              <button type="button" @click="showGoogleCustomForm = false" class="google-btn-secondary">Kembali</button>
              <button type="submit" class="google-btn-primary">Masuk</button>
            </div>
          </form>
        </div>
        
        <div class="google-modal-footer">
          <div class="google-footer-left">Indonesia</div>
...
const showMockGoogleModal = ref(false)
const showGoogleCustomForm = ref(false)
const googleCustomForm = reactive({ name: '', email: '' })
const mockGoogleAccounts = [
  { name: 'Rizki Darmawan', email: 'rizki@example.com', id: '111111111111111111111' },
  { name: 'Nayyara Aqila', email: 'nayyara@example.com', id: '222222222222222222222' },
  { name: 'Silvi Amalia', email: 'silvi@example.com', id: '333333333333333333333' },
]
...
function handleGoogleLogin() {
  ...
  if (clientId && clientId !== 'YOUR_GOOGLE_CLIENT_ID' && !clientId.includes('mockclientid')) {
    ...
    showMockGoogleModal.value = true
    showGoogleCustomForm.value = false
  }
}

async function selectMockGoogleAccount(acc) {
  showMockGoogleModal.value = false
  isLoading.value = true
  showToast('Menghubungkan dengan Google...')
  
  try {
    const payload = {
      id_token: `mock_google_token_${acc.id}`,
      email: acc.email,
      name: acc.name
    }
    const result = await authStore.loginWithGoogle(payload)
...
function submitGoogleCustomAccount() {
  const acc = {
    name: googleCustomForm.name,
    email: googleCustomForm.email,
    id: 'custom_' + Date.now()
  }
  selectMockGoogleAccount(acc)
}
...
.google-footer-right span {
  cursor: pointer;
}
.google-footer-right span:hover {
  color: #202124;
}
